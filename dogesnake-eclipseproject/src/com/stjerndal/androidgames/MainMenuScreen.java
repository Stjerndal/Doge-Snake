...
//                    LoadingScreen.loadDog(game);
                ...
                if(inBounds(event, V.main_signInButtonPositionX, V.main_signInButtonPositionY, V.main_signInButtonWidth, V.main_signInButtonHeight) ) {
                	if(Settings.soundEnabled)
                        Assets.click.play(1);
                    if(((AndroidGame) game).isSignedIn()) {
                    	((AndroidGame) game).signOut();
                    } else {
                    	((AndroidGame) game).signIn();
                    }
                    return;
                }
                ...
                if(inBounds(event, V.main_highscoresButtonPositionX, V.main_highscoresButtonPositionY, V.main_highscoresButtonWidth, V.main_highscoresButtonHeight) ) {
                	if(Settings.soundEnabled)
                        Assets.click.play(1);
                    if(((AndroidGame) game).isSignedIn()) {
                    	((AndroidGame) game).showLeaderboards();
                    } else {
                    	game.setScreen(new HighscoreScreen(game));
                    }
                    return;
                }
                if(inBounds(event, V.main_achievementsButtonPositionX, V.main_achievementsButtonPositionY, V.main_achievementsButtonWidth, V.main_achievementsButtonHeight) ) {
                	if(Settings.soundEnabled)
                        Assets.click.play(1);
//                	if(((AndroidGame) game).isSignedIn()) {
                		((AndroidGame) game).showAchievements();
//                    } else {
...
//                    }
    ...
    public void present(float deltaTime) {
        ...
        if(((AndroidGame) game).isSignedIn()) {
        	g.drawPixmap(Assets.signOut, V.main_signInButtonPositionX, V.main_signInButtonPositionY);
        } else {
        	g.drawPixmap(Assets.signInGplus, V.main_signInButtonPositionX, V.main_signInButtonPositionY);
        }
    }
