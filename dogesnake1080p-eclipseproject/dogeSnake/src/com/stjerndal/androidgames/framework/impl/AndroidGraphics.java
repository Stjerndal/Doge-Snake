...
//				(color & 0xff));
		canvas.drawARGB((color & 0xff000000) >> 24, (color & 0x00ff0000) >> 16, (color & 0x0000ff00) >> 8,
				(color & 0x000000ff));
	}
	...
	public void drawLine(int thickness, int x, int y, int x2, int y2, int color) {
		paint.setColor(color);
	...
	public void drawPixmap(Pixmap pixmap, int x, int y, int srcX, int srcY,
		...
		dstRect.left = x;
		dstRect.top = y;
		dstRect.right = (int) (x + (srcWidth*scale) - 1);
		dstRect.bottom = (int) (y + (srcHeight*scale) - 1);
